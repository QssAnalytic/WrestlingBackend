...
def stats_durability(session_factory, params: dict, obj:dict, db: Session):
    ...
    statement = text("""
        --durability_score
        with takedown_durability_score as (
        ...
        with fighter_matches as (
            ...
            select fighter_id, percentile as takedown_durability_score, percentile as bar_pct from (
                select *, round(1 - cast(tkd_rank as decimal) / cast(max(tkd_rank) over() as decimal), 2) percentile from(
            select *, rank() over(order by score_pct desc) tkd_rank from (
            select fighter_id, (tkd_bar_pct + bar_pct + count_bar_pct)/3 score_pct from takedown_success t
            left join avg_takedown_points_per_fight a
                on t.fighter_id = a.fighter
        ...
        select opponent_id, defense_score_durability, defense_score_durability as bar_pct from(
        select *, round(1 - cast(defense_rank as decimal) / cast(max(defense_rank) over() as decimal), 2) defense_score_durability from(
        select *, rank() over(order by bar_pct desc) defense_rank from(
        select opponent_id, bar_pct from action_escape_rate)))),
        offense_durability_score as (
        --offense_durability_score
        with action_success_rate as(
        with total as (
                select f.fighter_id, count(*) as total_count from fightstatistics f
                inner join fightinfos f2 on f.fight_id = f2.id
        ...
        durability_score as (
        select coalesce(t.fighter_id, opponent_id, o.fighter_id) fighter, coalesce(takedown_durability_score, 0) takedown_durability_score, 
        coalesce(defense_score_durability, 0) defense_score_durability, 
        coalesce(offense_score, 0) offense_score
        from takedown_durability_score t 
        full outer join defense_score_durability d
        on t.fighter_id = d.opponent_id
        full outer join offense_durability_score o
        on t.fighter_id = o.fighter_id),
        durability_percentile as (
        select *, percent_rank() over(order by calc asc) from (
        select fighter, round((takedown_durability_score + defense_score_durability + offense_score) / 3, 2)  as calc
        from durability_score))
        select fighter, calc durability_score, round(cast(percent_rank as decimal), 2) durability_percentile from durability_percentile where fighter = :fighter_id
